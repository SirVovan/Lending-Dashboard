...
IMPORTANT: все исправленные баги хранятся в `docs/BUGS.md`. Читать перед генерацией новой админки и перед доработкой. После каждого нового исправления — добавлять запись туда.

### Фиксация сессии

Когда пользователь говорит «фиксируй сессию», «сохрани сессию в память», «сохрани в хранилище» или аналогичное:

IMPORTANT: после записи в auto-memory и Vibecoding Vault — выполнить коммит и пуш в GitHub:

```bash
git add -A
git commit -m "<краткое описание изменений сессии>"
git push
```

Коммитить только файлы репозитория (CLAUDE.md, docs/, _template/). Папка `projects/` в .gitignore — не коммитится.
